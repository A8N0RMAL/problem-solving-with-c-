...
vector<bool> is_prime_cache;

// Function to compute the first 50 Fibonacci numbers
...
// Precompute the prime status for the first 50 Fibonacci numbers
void precomputePrimeStatus() {
    is_prime_cache.resize(MAX_N + 1);
    for (int i = 1; i <= MAX_N; ++i) {
        is_prime_cache[i] = isPrime(fibonacci[i]);
    }
}
int test()
{
    compute_Fibonacci();
    precomputePrimeStatus();
    c(T);
    while(T--)
    {
        c(n);
        cout<<((is_prime_cache[n])?"prime\n":"not prime\n");
    }
}
int32_t main()
{
