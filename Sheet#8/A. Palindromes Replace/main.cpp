...
string Make_Smallest_Palindrome(string s)
{
    ...
    while(left <= right)
    {
        ...
        {
            if(s[left] == 63)
                s[left] = s[right] = 97; // Replace both '?' with 'a'
        }
        else if(s[left] == 63)
            s[left] = s[right]; // Replace '?' at left with character at right
        else if(s[right] == 63)
            s[right] = s[left]; // Replace '?' at right with character at left
        else
            return "-1"; // Characters are different and none is '?'
        ++left, --right;
    }
    return s;
}
int test()
{
    string s;
    cin>>s;
    string result = Make_Smallest_Palindrome(s);
    cout<<result;
}
int32_t main()
{
