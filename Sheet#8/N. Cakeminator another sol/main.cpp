...
int test()
{
    ...
    // Identify rows and columns with 'S'
    for (int i = 0; i < r; ++i) {
        for (int j = 0; j < c; ++j) {
            if (cake[i][j] == 'S') {
    ...
    int edibleCells = 0;
    // Count edible cells in rows without 'S'
    for (int i = 0; i < r; ++i) {
    ...
    // Count edible cells in columns without 'S', avoiding double-counting
    for (int j = 0; j < c; ++j) {
