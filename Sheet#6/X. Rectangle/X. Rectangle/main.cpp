...
int test()
{
    ...
    ll mn_x=min(min(x1, x2), min(x3, x4));
    ll mx_x=max(max(x1, x2), max(x3, x4));
    ll mn_y=min(min(y1, y2), min(y3, y4));
    ll mx_y=max(max(y1, y2), max(y3, y4));

    c(n);
    fl(i, n)
    {
        c(x);c(y);
        cout<<(x>=mn_x && x<=mx_x && y>=mn_y && y<=mx_y ?ye:no);
    }
}
int32_t main()
{
