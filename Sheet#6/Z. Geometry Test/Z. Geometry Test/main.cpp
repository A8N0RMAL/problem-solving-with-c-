...
{
    c(r);c(s);
    // The circle can hold the square if the diameter of the circle is at least as large as the diagonal of the square.
    // The diameter of the circle is 2R
    // The diagonal of the square is s*sqrt(2)
    // Check if 2R >= s*sqrt(2)
    if(r*sqrt(2) >= s)
        cout<<"Circle\n";
    // Square Holds Circle: S >= 2R
    else if(s >= 2*r)
        cout<<"Square\n";
    else
        cout<<"Complex\n";
