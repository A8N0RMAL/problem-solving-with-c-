...
int test()
{
    ...
    cin>>n>>x;
    ll remainder=0;
    // Convert the digit from its character form to an integer.
    // Update the remainder using this formula
    for(char digit:n)
        remainder=(remainder*10+(digit-'0'))%x;
    // Check Divisibility: After processing all digits, if remainder is 0, N is divisible by X
    if(remainder == 0)
        cout<<ye;
    else
        cout<<no;
