...
void read_matrix(vector<vector<int>>&matrix, ll rows, ll cols)
{
    fl(i, rows)
    {
        fl(j, cols)
...
void add_matrices_recursive(const vector<vector<int>>&A, const vector<vector<int>>&B, vector<vector<int>>&result, ll i, ll j, ll R, ll C)
{
    if(i == R)return;
    if(j == C)
    {
        add_matrices_recursive(A, B, result, i+1, 0, R, C);
        return;
    }
    result[i][j] = A[i][j] + B[i][j];
    add_matrices_recursive(A, B, result, i, j+1, R, C);
}
...
int test()
{
    ...
    add_matrices_recursive(A, B, result, 0, 0, R, C);
